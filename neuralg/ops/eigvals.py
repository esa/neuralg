...
def eigvals(A, symmetric=False, real=False):
   ...
   Returns:
    ...
    if symmetric:
        model = neuralg_ModelHandler.get_model("eigval", matrix_size)
    elif real:
        model = neuralg_ModelHandler.get_model("r_eigval", matrix_size)
    else:
        model = neuralg_ModelHandler.get_model("c_eigval", matrix_size)
