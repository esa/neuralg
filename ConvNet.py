...
class EigConvNet(nn.Module):
    def __init__(self, matrix_dimension,hidden_layers, filters, kernel_size):
        super(EigConvNet, self).__init__()
        self.net = []
        self.net.append(nn.Conv2d(1,filters,kernel_size, padding = "same"))
        self.net.append(nn.BatchNorm2d(filters))
        self.net.append(nn.ReLU())
        ...
        self.net.append(nn.Linear(matrix_dimension**2, matrix_dimension))
        self.net = nn.Sequential(*self.net)
    def forward(self, x):
        out = self.net(x)
        return out
